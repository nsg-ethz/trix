...
//! Extract the time series of forwarding state udpate from the bgp (prefixes) log.
...
use trix::{
    analyzer::{
        bgp_log_parser::{BgpPrefixesRecord, PrefixesLineKind},
        CiscoAnalyzerData,
    },
...
use super::{Error, ParseableRecord, TransformParseableRecord, UpdateKind};

#[allow(dead_code)]
const MIN_DELTA: f64 = 0.0001;

impl ParseableRecord for BgpPrefixesRecord {
    fn time(&self) -> f64 {
        self.time
    }
    ...
    fn kind(&self) -> Option<UpdateKind> {
        match self.kind {
            PrefixesLineKind::NewBestPath => Some(UpdateKind::Add),
            PrefixesLineKind::MarkForDeletion => Some(UpdateKind::Del),
            PrefixesLineKind::BRibAdd => None,
        }
    }

    fn ospf_next_hop(&self) -> Option<Ipv4Addr> {
        self.new_best_next_hop
    }
}
...
pub(crate) fn process_sample(
    ...
    if !filename.exists() {
        log::warn!(
            "BGP prefixes log doesn't exist! Skipping this sample. Path: {}",
    ...
    for record in csv::Reader::from_path(&filename)?.into_deserialize() {
        let record: BgpPrefixesRecord = record?;
