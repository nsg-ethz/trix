...
use trix::analyzer::analyzer_script::*;
use bgpsim::{
...
async fn main() -> R {
    ...
    let geo_timing_model = GeoTimingModel::<SinglePrefix>::new(
    ...
    for _ in 0..num_samples {
        ...
        lab.advertise_route(
            ...
            &BgpRoute {
                prefix: SinglePrefix,
                as_path: vec![100.into(), 100.into(), 1000.into()],
                next_hop: e1,
                local_pref: None,
                med: None,
        ...
        lab.advertise_route(
            ...
            &BgpRoute {
                prefix: SinglePrefix,
                as_path: vec![100.into(), 100.into(), 100.into(), 100.into(), 1000.into()],
                next_hop: e1,
                local_pref: None,
                med: None,
                community: Default::default(),
    ...
    let (traffic_pcap_path, traffic_monitor_handle) = lab
        .start_traffic_monitor(format!("monitor_{}", "update_e1"), true)
        .await?;
    ...
    for sample_idx in 0..num_samples {
        ...
        log::debug!("[analyzer] Execute event {sample_idx}: updating best route");
...

        {
            ...
            analyzer_set.spawn(async move {
                ...
                ssh.execute_cmd(&[&format!("echo \"{:?}\" >> data_update_e1.csv", result)])
                    .await
        ...
        log::debug!("[analyzer] Reset experiment: announcing worse route");
        lab.get_exabgp_handle().step().await?;
...
async fn evaluate_trace(analyzer_output: String) -> R<(f64, f64)> {
    ...
    let e1r0_bgp = messages
        .iter()
        .find(|msg| msg.src_rid == "e1" && msg.dst_rid == "r0" && msg.msg_type == "BGP")
        .ok_or("e1 needs to send a BGP update to r0")?;

    log::trace!("[analyzer] found bgp message from e1 to r0: {e1r0_bgp:#?}");

    // find second occurrence of r0's BGP message to r1
    let r0r1_bgp = messages
        .iter()
        .filter(|msg| msg.timestamp >= e1r0_bgp.timestamp)
        .filter(|msg| msg.src_rid == "r0" && msg.dst_rid == "r1" && msg.msg_type == "BGP")
        .nth(1)
        .ok_or("r0 needs to send a BGP update to r1 that passes the delayer once")?;

    log::trace!("[analyzer] found bgp message from r0 to r1: {r0r1_bgp:#?}");

    // find first occurrence of r1's response BGP message to r0
    let r1_bgp_reply = messages
        .iter()
        .filter(|msg| msg.timestamp >= r0r1_bgp.timestamp)
        .find(|msg| msg.src_rid == "r1" && msg.dst_rid == "e3" && msg.msg_type == "BGP")
        .ok_or("r1 needs to send a BGP update to its peers")?;

    log::trace!("[analyzer] found bgp message from r1 to a peer: {r1_bgp_reply:#?}");

    // ensure that the network had the expected forwarding state when the BGP message arrives
    let r1r2_probe = messages
        .iter()
        .filter(|msg| msg.timestamp >= r0r1_bgp.timestamp)
        .find(|msg| msg.src_rid == "r1" && msg.dst_rid == "r2" && msg.msg_type == "PROBE")
        .ok_or("r1 should still send traffic to r2 for now")?;

    log::trace!("[analyzer] found probe message from r1 to r2: {r1r2_probe:#?}");

    // find the first prober packet being routed on the new link
    let r1r0_probe = messages
        .iter()
        .filter(|msg| msg.timestamp >= r1r2_probe.timestamp)
        .find(|msg| msg.src_rid == "r1" && msg.dst_rid == "r0" && msg.msg_type == "PROBE")
        .ok_or("r1 should send traffic to r0 now")?;

    log::trace!("[analyzer] found probe message from r1 to r0: {r1r0_probe:#?}");

    log::trace!(
        "[analyzer] cp reaction: {:.3?}ms, dp reaction: {:.3?}ms",
        (r1_bgp_reply.timestamp - r0r1_bgp.timestamp) * 1000.0,
        (r1r0_probe.timestamp - r0r1_bgp.timestamp) * 1000.0
    );
    Ok((
        (r1_bgp_reply.timestamp - r0r1_bgp.timestamp) * 1000.0,
        (r1r0_probe.timestamp - r0r1_bgp.timestamp) * 1000.0,
