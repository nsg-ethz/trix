...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    loop {
        ...
        for topo_dir in topos {
            let scenarios = fs::read_dir(topo_dir.unwrap().path().display().to_string()).unwrap();
            for scenario in scenarios {
                let mut scenario_path = scenario.unwrap().path();

                if !scenario_path.to_string_lossy().to_string().contains("Path") {
                    log::trace!("Skipping scenario from {scenario_path:?}.");
                    continue;
                }
                ...
                while let Err(e) = analyzer.analyze_router_lab(100, 5, 2000, &data_path).await {
                    ...
                    if error_message.contains("CannotObtainLock")
                        && error_message.contains("roschmi")
                    {
