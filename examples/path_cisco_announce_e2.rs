...
use trix::{analyzer::analyzer_script::*, Prefix as P};
use bgpsim::{
...
fn set_conf_dir() -> R {
    ...
    conf_dir.pop(); // move out of `examples/`
    conf_dir.pop(); // move out of `release/`
...
async fn main() -> R {
    ...
    let (net, (r0, r1, r2, _e1, e2)) = net! {
        ...
        routes = {
            e1 -> prefix as {path: [100, 100, 100, 100, 1000]};
        };
    ...
    let geo_timing_model = GeoTimingModel::<P>::new(
    ...
    for _ in 0..num_samples {
        ...
        lab.advertise_route(
            ...
            &BgpRoute {
                prefix,
                as_path: vec![200.into(), 200.into(), 1000.into()],
                next_hop: e2,
                local_pref: None,
                med: None,
    ...
    let (traffic_pcap_path, traffic_monitor_handle) = lab
        .start_traffic_monitor(format!("monitor_{}", "announce_e2"), true)
        .await?;
    ...
    for sample_idx in 0..num_samples {
        ...
        log::debug!("[analyzer] Execute event {sample_idx}: announcing best route");
...

        {
            ...
            analyzer_set.spawn(async move {
                ...
                ssh.execute_cmd(&[&format!("echo \"{:?}\" >> data_announce_e2.csv", result)])
                    .await
        ...
        log::debug!("[analyzer] Reset experiment: withdrawing new best route");
        lab.get_exabgp_handle().step().await?;
...
async fn evaluate_trace(analyzer_output: String) -> R<(f64, f64)> {
    ...
    let r0e1_probe = messages
        ...
        .ok_or("r0 should still send traffic to e1 for now")?;
    ...
    let r0r1_probe = messages
        .iter()
        .filter(|msg| msg.timestamp >= r0e1_probe.timestamp)
        .find(|msg| msg.src_rid == "r0" && msg.dst_rid == "r1" && msg.msg_type == "PROBE")
        .ok_or("r0 should send traffic to r1 now")?;
    ...
    log::trace!(
        ...
        (r0r1_probe.timestamp - r2r0_bgp.timestamp) * 1000.0
    );
    Ok((
        (r0_bgp_reply.timestamp - r2r0_bgp.timestamp) * 1000.0,
        (r0r1_probe.timestamp - r2r0_bgp.timestamp) * 1000.0,
